      ...
      if (typeRhs.ElementInit == null) {
        ...
        for (var d = 0; d < typeRhs.ArrayDimensions.Count; d++) {
          w = CreateForLoop(indices[d], w => {
            EmitArrayLength(w => EmitIdentifier(nw, wr), typeRhs, d, w);
          }, w);
        }
      ...
      if (expr is LiteralExpr) {
        ...
        for (var i = 0; i < dtv.Arguments.Count; i++) {
          ...
          if (!formal.IsGhost) {
            ...
            var w = EmitCoercionIfNecessary(@from: dtv.Arguments[i].Type, to: dtv.Ctor.Formals[i].Type.Subst(typeSubst), tok: dtv.tok, wr: wrArgumentList);
    ...
    protected virtual void CompileFunctionCallExpr(FunctionCallExpr e, ConcreteSyntaxTree wr, bool inLetExprBody,
      ...
      for (int i = 0; i < e.Args.Count; i++) {
        if (!e.Function.Formals[i].IsGhost) {
          ...
          var w = EmitCoercionIfNecessary(fromType, instantiatedToType, tok: e.tok, wr: wr);
