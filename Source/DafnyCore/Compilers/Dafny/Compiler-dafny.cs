  ...
  class DafnyCompiler : SinglePassCompiler {
    ...
    protected override IClassWriter DeclareNewtype(NewtypeDecl nt, ConcreteSyntaxTree wr) {
      if (currentBuilder is NewtypeContainer builder) {
        ...
        return new ClassWriter(this, builder.Newtype(nt.GetCompileName(Options), GenType(nt.BaseType), true, witnessStmts, witness));
      } else {
        throw new InvalidOperationException();
      }
    }
    ...
    protected override void DeclareSubsetType(SubsetTypeDecl sst, ConcreteSyntaxTree wr) {
      if (currentBuilder is NewtypeContainer builder) {
        ...
        builder.Newtype(sst.GetCompileName(Options), GenType(sst.Rhs), sst.Rhs.IsNumericBased(), witnessStmts, witness).Finish();
      } else {
        throw new InvalidOperationException();
      }
    }
