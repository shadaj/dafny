...
module {:extern "DCOMP"} DCOMP {
  ...
  class COMP {
    ...
    static method GenClass(c: Class, path: seq<Ident>) returns (s: string) {
      ...
      s := s + "\n" + "impl " + constrainedTypeParams + " " + escapeIdent(c.name) + typeParams + " {\n" + implBody + "\n}";
      if (|c.superClasses| > 0) {
        var i := 0;
        while i < |c.superClasses| {
          ...
          match superClass {
            case Path(traitPath, typeArgs, Trait(_)) => {
              ...
              s := s + "\nimpl " + constrainedTypeParams + pathStr + typeArgs + " for ::std::rc::Rc<" + genSelfPath + typeParams + "> {\n" + body + "\n}";
            }
            case _ => {}
          }
          i := i + 1;
      ...
      var defaultImpl := "impl " + constrainedTypeParams + " ::std::default::Default for " + escapeIdent(c.name) + typeParams + " {\n";
      ...
      var ptrPartialEqImpl := "impl " + constrainedTypeParams + " ::std::cmp::PartialEq for " + escapeIdent(c.name) + typeParams + " {\n";
    ...
    static method GenNewtype(c: Newtype) returns (s: string) {
      ...
      s := s + "#[inline]\nfn unerase<'a>(x: &'a " + underlyingType + ") -> &'a Self {\n";
      s := s + "unsafe { &*(x as *const " + underlyingType + " as *const " + escapeIdent(c.name) + typeParams + ") }\n";
      s := s + "}\n";
      s := s + "#[inline]\nfn unerase_owned(x: " + underlyingType + ") -> Self {\n";
      s := s + escapeIdent(c.name) + "(x)\n";
      s := s + "}\n";
      s := s + "}\n";
      ...
      s := s + "#[inline]\nfn unerase<'a>(x: &'a " + escapeIdent(c.name) + typeParams + ") -> &'a Self {\n";
      s := s + "x\n";
      s := s + "}\n";
      s := s + "#[inline]\nfn unerase_owned(x: " + escapeIdent(c.name) + typeParams + ") -> Self {\n";
      s := s + "x\n";
      s := s + "}\n";
      s := s + "}\n";
      s := s + "impl " + constrainedTypeParams + " ::std::default::Default for " + escapeIdent(c.name) + typeParams + " {\n";
      s := s + "fn default() -> Self {\n";
    ...
    static method GenDatatype(c: Datatype) returns (s: string) {
      ...
      identEraseImpls := identEraseImpls + "impl " + constrainedTypeParams + " ::dafny_runtime::DafnyUnerasable<" + escapeIdent(c.name) + typeParams + "> for " + escapeIdent(c.name) + typeParams + " {\n" + "fn unerase(x: &Self) -> &Self { x }\nfn unerase_owned(x: Self) -> Self { x }}\n";
    ...
    static method GenExpr(e: Expression, selfIdent: Optional<string>, params: seq<string>, mustOwn: bool) returns (s: string, isOwned: bool, isErased: bool, readIdents: set<string>)
      ...
      match e {
        ...
        case DatatypeValue(path, variant, isCo, values) => {
          var path := GenPath(path);
          s := "::std::rc::Rc::new(" + path + "::" + escapeIdent(variant);
          readIdents := {};
